...
#define GOTURN_MODEL_PATH "/data/c610gst/models/goturn_quantized.dlc"
#endif

#define MODEL_INPUT_W 227
#define MODEL_INPUT_H 227
...
std::vector<int> ObjectTracker::doDetect(std::vector<cv::Mat>& mat_vec) 
{
    assert(mat_vec.empty() != true);
    ...
    std::cout << __FILE__ << "do detect processing..." << __LINE__ << std::endl;
    ...
    for( auto& name: tensorNames ){
        ...
        cout << "OutputShape : " << outputTensor->getShape().getDimensions()[0] << " x "
            << outputTensor->getShape().getDimensions()[1] << endl;
    }
